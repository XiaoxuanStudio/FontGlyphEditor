(() => {
  ...
  function adjustPanel() {
    return `<div class="panel">
      ...
      ${state.adjustment.scope === 'selected' ? `<label>指定字符<input data-path="adjustment.selected_chars" value="${escapeHtml(state.adjustment.selected_chars)}" placeholder="例如：の雨70" /></label>` : ''}
  ...
  function colorPanel() {
    return `<div class="panel">
      ...
      ${state.color.scope === 'selected' ? `<label>指定字符<input data-path="color.selected_chars" value="${escapeHtml(state.color.selected_chars)}" placeholder="例如：の雨70" /></label>` : ''}
      <label>颜色模式
    ...
    return `<div class="patch-row">
      ...
      <div>
        ...
        <div class="patch-controls">
          <label>替换字符<input data-patch="${p.id}" data-field="character" value="${escapeHtml(p.character)}" placeholder="如 の" /></label>
